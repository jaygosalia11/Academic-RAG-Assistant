...
//     try {
//       await createSession(newId, user.id);
//     } catch (err) {
...
//         overflow: "hidden",
//         background:
//           "linear-gradient(145deg, #0a0e1a 0%, #0d1425 40%, #0a1020 70%, #080c18 100%)",
//         "&::before": {
//           content: '""',
//           position: "fixed",
//           inset: 0,
...
//             sx={{
//               color: "rgba(165,180,252,0.6)",
//               "&:hover": { color: "rgba(165,180,252,1)" },
//             }}
//           >
...
//         <Container
//           maxWidth="lg"
//           sx={{ position: "relative", zIndex: 2, py: 5 }}
//         >
//           <Box sx={{ mb: 4 }}>
//             <Typography
//               variant="h4"
...
const ChatPage = () => {
  ...
  return (
    ...
    >
      ...
      {/* Hamburger — top left */}
      <Box onClick={() => setDrawerOpen(true)} sx={{ position: "fixed", top: { xs: 12, sm: 16 }, left: { xs: 12, sm: 16 }, zIndex: 10, cursor: "pointer", width: { xs: 36, sm: 40 }, height: { xs: 36, sm: 40 }, borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: { xs: 17, sm: 20 }, color: "rgba(165,180,252,0.9)", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(165,180,252,0.12)", backdropFilter: "blur(12px)", transition: "all 0.2s ease", "&:hover": { background: "rgba(99,102,241,0.18)" } }}>
        ☰
      </Box>

      {/* Logout — top right */}
      <IconButton onClick={handleLogout} title="Logout" sx={{ position: "fixed", top: { xs: 8, sm: 12 }, right: { xs: 12, sm: 16 }, zIndex: 10, width: { xs: 36, sm: 40 }, height: { xs: 36, sm: 40 }, color: "rgba(165,180,252,0.6)", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(165,180,252,0.12)", backdropFilter: "blur(12px)", "&:hover": { background: "rgba(239,68,68,0.15)", color: "#f87171" }, transition: "all 0.2s ease" }}>
        <LogoutIcon sx={{ fontSize: { xs: 18, sm: 20 } }} />
      </IconButton>

      {/* Drawer */}
      <Drawer anchor="left" open={drawerOpen} onClose={() => setDrawerOpen(false)} slotProps={{ paper: { sx: { width: { xs: "75vw", sm: 280 }, background: "rgba(10,14,26,0.97)", backdropFilter: "blur(24px)", color: "white", borderRight: "1px solid rgba(165,180,252,0.1)", boxShadow: "4px 0 32px rgba(0,0,0,0.5)", display: "flex", flexDirection: "column" } } }}>
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", p: 2, borderBottom: "1px solid rgba(165,180,252,0.08)" }}>
          <Typography sx={{ color: "rgba(165,180,252,0.85)", fontWeight: 700, fontSize: "0.95rem" }}>💬 Chats</Typography>
          <IconButton onClick={() => setDrawerOpen(false)} size="small" sx={{ color: "rgba(165,180,252,0.6)" }}>
            <CloseIcon fontSize="small" />
          </IconButton>
        </Box>

        <Box onClick={() => { handleNewChat(); setDrawerOpen(false); }} sx={{ mx: 2, mt: 2, mb: 1, p: 1.5, borderRadius: "10px", textAlign: "center", cursor: "pointer", background: "rgba(99,102,241,0.18)", border: "1px solid rgba(99,102,241,0.28)", color: "rgba(165,180,252,0.9)", fontWeight: 600, fontSize: "0.875rem", transition: "all 0.2s ease", "&:hover": { background: "rgba(99,102,241,0.28)" } }}>
          + New Chat
        </Box>
      ...
      {/* ── Body ── */}
      <Box sx={{ position: "relative", zIndex: 2, flex: 1, minHeight: 0, display: "flex", flexDirection: "column", pt: { xs: "56px", sm: "64px" } }}>
        <Container maxWidth="lg" sx={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column", px: { xs: 1.5, sm: 3 }, pb: { xs: 2, sm: 3 } }}>
          ...
          {pageLoading ? (
            ...
            <Box sx={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", gap: 2 }}>
              ...
              <Typography sx={{ color: "rgba(165,180,252,0.7)", fontWeight: 600, fontSize: { xs: "1.1rem", sm: "1.3rem" }, textAlign: "center" }}>
                Welcome to AcademIQ
              </Typography>
              <Typography sx={{ color: "rgba(165,180,252,0.35)", fontSize: "0.875rem", textAlign: "center", maxWidth: 300 }}>
                Start a new chat to ask anything from your syllabus.
              </Typography>
              <Button
                variant="contained"
                startIcon={<AddCommentIcon />}
                onClick={handleNewChat}
                sx={{ mt: 1, px: 4, py: 1.4, borderRadius: "12px", textTransform: "none", fontWeight: 600, fontSize: "0.95rem", background: "linear-gradient(135deg, #6366f1, #4f46e5)", boxShadow: "0 4px 20px rgba(99,102,241,0.4)", "&:hover": { background: "linear-gradient(135deg, #818cf8, #6366f1)" } }}
              >
            ...
            <Box sx={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
              ...
              >
                {messages.length === 0 ? (
                  <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", mt: 8, gap: 1 }}>
                    ...
                    <Typography sx={{ color: "rgba(165,180,252,0.35)", fontSize: "0.875rem" }}>
                      Ask anything from your syllabus.
                    </Typography>
                  </Box>
                ) : (
              ...
              <Box sx={{ pt: 1 }}>
                <ChatInput
                  ...
                  disabled={loading}
                />
                <Typography sx={{ fontSize: "0.72rem", color: wordCount >= WORD_LIMIT ? "#f87171" : "rgba(255,255,255,0.25)", textAlign: "right", pr: 1, mt: 0.5 }}>
